import { api } from "@/api";
import { CreateCategoryDto, UpdateCategoryDto, CategoryBackendDto } from "./dto/categories.dto";
...
class CategoriesService {
    ...
    async createCategories(data: CreateCategoryDto[]): Promise<CategoryBackendDto[]> {
        const response = await api.post<CategoryBackendDto[]>(RESOURCES.CREATE, data);
        return response.data;
    }

    async updateCategory(id: string, data: UpdateCategoryDto): Promise<CategoryBackendDto> {
        const response = await api.patch<CategoryBackendDto>(RESOURCES.UPDATE(id), data);
        return response.data;
    }
    ...
    async getCategoriesByStatus(isArchived: boolean): Promise<CategoryBackendDto[]> {
        const response = await api.get<CategoryBackendDto[]>(RESOURCES.BY_STATUS(isArchived));
        return response.data;
    }

    async searchCategories(query: string): Promise<CategoryBackendDto[]> {
        const response = await api.get<CategoryBackendDto[]>(RESOURCES.SEARCH(query));
        return response.data;
    }
